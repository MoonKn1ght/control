...
Tracking::Tracking(Chassis* chassis, CCD* ccd,PID* tracking_PID)
{
...
void Tracking::Handler()
{
	...
	for(int i = 0; i < 128; ++i)
	{
		data_norm[i] = ccd->data[i]/(ccd_norm[i]/2663.08);
	}
	...
	else
	{
		...
		x_path = chassis->x + distance*cos(theta + chassis->ang*PI/180);
		y_path = chassis->y + distance*sin(theta + chassis->ang*PI/180);
		if(Right - Left < 35)
		{
			chassis->v_set = v ;
